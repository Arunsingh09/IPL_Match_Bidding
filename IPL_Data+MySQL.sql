...
select bd.bidder_id,bd.bidder_name,(bd_won.wins/no_of_bids)*100 as percentage_of_wins from ipl_bidder_details bd
join ipl_bidder_points bp
on bd.bidder_id = bp.bidder_id
inner join (select bidder_id, count(bid_status) as wins from ipl_bidding_details where bid_status = 'won'group by bidder_id) as bd_won
on bp.bidder_id = bd_won.bidder_id
group by bd.bidder_id,bd.bidder_name order by percentage_of_wins desc;
...
select match_id, win_details , toss_winner, match_winner,  ((toss_winner)/sum(match_winner))*100 as "percentage of win by a team" 
from ipl_match group by match_winner;

#4.	Show the total bids along with bid team and team name.
select count(*), bid_team, team_name 
from ipl_bidding_details ipl_bd
join ipl_team ipl_t
on ipl_bd.bid_team =ipl_t.team_id
group by bid_team, team_name
order by bid_team;

#5.	Show the team id who won the match as per the win details.

SELECT TEAM_ID2 AS TEAM_ID, WIN_DETAILS ,MATCH_WINNER
FROM IPL_MATCH 
...
SELECT IT.TEAM_ID,TEAM_NAME, PLAYER_ROLE,COUNT(IT.TEAM_ID) "TOTAL ALL_Rounder"
FROM IPL_TEAM IT
JOIN IPL_TEAM_PLAYERS ITP
ON IT.TEAM_ID= ITP.TEAM_ID
WHERE PLAYER_ROLE LIKE "ALL-Rounder"
GROUP BY IT.TEAM_ID
HAVING COUNT(IT.TEAM_ID)>4;
